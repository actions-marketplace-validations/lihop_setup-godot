...
class Windows extends Platform {
  ...
  async postDownload() {
    ...
    const mesaAsset = `mesa3d-${MESA_DIST_WIN_VERSION}-release-msvc.7z`;
    const mesaUrl = `https://github.com/pal1000/mesa-dist-win/releases/download/${MESA_DIST_WIN_VERSION}/${mesaAsset}`;
    await tc.downloadTool(mesaUrl, `${outputdir}/${mesaAsset}`);
